import React, { useRef } from "react";
import lang from "../utils/languageConstaints";
import { useDispatch, useSelector } from "react-redux";
import openai from "../utils/openai";
import { API_OPTIONS } from "../utils/constants";
import { addMovieResults } from "../utils/gptSlice";

const GptSearchBar = () => {
  const Lankey = useSelector((store) => store.config.lang);
  const searchText = useRef();
  const dispatch = useDispatch();

  const searchMovieTmdb = async (movie) => {
    const data = await fetch(
      "https://api.themoviedb.org/3/search/movie?query=" +
        movie +
        "&include_adult=false&language=en-US&page=1",
      API_OPTIONS
    );
    const json = await data.json();
    return json.results;
  };

  const handleGptSearchClick = async () => {
    const gptQuery =
      "Act as Movie Recommendation system and suggest some movie for the query : " +
      searchText.current.value +
      ". only give me name of 5 movie, comma seperated like the example result given ahead. Example Result: Gadar, sholay, Don, Golmaal, koi Mil gaya";
    // api call
    const gptResult = await openai.chat.completions.create({
      messages: [{ role: "user", content: gptQuery }],
      model: "gpt-3.5-turbo",
    });

    if (!gptResult.choices) {
      // make Error handing page
    }

    // Andaz Apna Apna, Chupke Chupke, Padosan, Chalti Ka Naam Gaadi, Jaane Bhi Do Yaaro

    const gptMovies = gptResult.choices?.[0]?.message?.content.split(",");

    // [Andaz Apna Apna, Chupke Chupke, Padosan, Chalti Ka Naam Gaadi, Jaane Bhi Do Yaaro]

    // for each movie I will search tmdb api
    const promiseArray = gptMovies.map((movie) => searchMovieTmdb(movie));

    
    // [promises, promises, promises, promises, promises]
    const tmdbResults = await Promise.all(promiseArray);

    dispatch(
      addMovieResults({ movieNames: gptMovies, movieResults: tmdbResults })
    );
  };

  return (
    <div className="pt-[43%] md:pt-[10%] flex justify-center">
      <form
        className="w-full md:w-1/2 bg-black grid grid-cols-12"
        onSubmit={(e) => e.preventDefault()}
      >
        <input
          type="text"
          className="p-4 m-4 col-span-9"
          placeholder={lang[Lankey].gptSearchPlaceholder}
          ref={searchText}
        ></input>
        <button
          className="col-span-3 m-4 py-2 px-4 bg-red-700 text-white rounded-lg"
          onClick={handleGptSearchClick}
        >
          {lang[Lankey].search}
        </button>
      </form>
    </div>
  );
};

export default GptSearchBar;
